...
| --- | --- |
| [Semantic Circuit Object Part I](#semantic-circuit-object-part-i-creating-symbols-and-using-them-directly-in-flux2) | Strongest synthesis: a causal semantic route is turned into a typed object symbol that can be read, edited, isolated, and written back into the model. The blue-mug value-level write works at both tested seeds, and durable manifest readback passes 11/11 checks. |
| [Semantic Circuit Object Part II](#semantic-circuit-object-part-ii-objects-become-debugger-io) | Image regions are mapped to lexical addresses, selective edits are demonstrated, the address is packaged as durable state, and dose, wrong-address writes, composition, and isolation are stress-tested. Fox recolor reaches 0.92/0.94 progress across two seeds, making this the clearest bridge from model internals to a usable control surface. |
| [Semantic Circuit Object Part III](#semantic-circuit-object-part-iii-semantic-object-registers-across-flux) | The semantic object is tested across FLUX.2 base/distilled/9B and FLUX.1 Schnell. The interface persists as a typed contract, while address grain and payload geometry change with the conditioner. |
...
### [Semantic Circuit Object Part I: Creating Symbols and Using Them Directly in FLUX.2](demos/semantic-circuit-object-part-I.md)
...
### [Semantic Circuit Object Part II: Objects Become Debugger I/O](demos/semantic-circuit-object-part-II.md)
...
### [Semantic Circuit Object Part III: Semantic Object Registers Across FLUX](demos/semantic-circuit-object-part-III.md)

The cross-model follow-up asks whether the object survives checkpoint, scale, and conditioner changes. FLUX.2 exposes sharp Qwen lexical rows, while FLUX.1 exposes noun-phrase windows; cross-conditioner wrong-object writes, value algebra, and manifest write-back preserve the object grammar while keeping address and payload geometry model-local. A model-family portability trend is established, not a universal shared tensor basis.
