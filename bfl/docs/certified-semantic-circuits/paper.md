...
This paper reports what happened when we imposed that battery on a modern production image generator. We chose a diffusion transformer rather than a language model for three reasons. Diffusion models iterate: the same weights execute several times over an evolving state, so "where" a computation lives has a temporal dimension that single-pass analyses miss. Diffusion models have a natural end-to-end observable — the rendered image — which lets us demand that every internal claim be confirmed at an output the model actually produces. And text-to-image models expose a clean interface between two subsystems (a text encoder and an image denoiser), which makes a rare experiment possible: swapping one subsystem entirely while holding the other frozen, then asking which side owns a behavior.
...
Correction to row 13: although the frozen prompt contrast was upright → folded ears, pane-level review shows the controlled rendered feature is eye openness/eyelid state (open → narrowed/closed); the ears remain pointed. The historical route-certified tier is retained as a route-level measurement, but it is not an established ear semantic circuit. See the [additive correction](corrections/ears-upright-folded-eye-control.md).
...
**Held-out-prompt replication.** Seeds are not the only thing the panel could have overfit; the prompts were fixed during exploration too. We therefore wrote two new contrasts for each axis that change the subject and scene wording while keeping the semantic distinction (for example, identity cat → fox becomes squirrel → rabbit on a forest stump), froze them with the same route, thresholds and gates before running ([preregistration](experiments/2026-09-22-prompt-heldout/PREREGISTRATION.md)), and ran three seeds (4242, 9001 and an unopened 20260922). An axis replicates if it passes the six causal-route gates on at least half of its six prompt × seed cells; the route survives prompt hold-out if at least four of the six strict axes replicate. So far two strict axes have run, and both replicate: identity passes the route gates on 5 of 6 held-out cells (Figure 4) and fur length on 5 of 6. The strict pixel tier replicates for identity (3 of 6 cells) but not for fur (1 of 6). Across all eight original axes run so far, including route-certified and open ones, seven replicate at the route level; the one that does not, sitting → lying pose, was already an open axis in the original panel. Orientation, beside → behind, lighting and weather: TODO-{flux-heldout-groups-4-5-8}. Aggregate verdict (≥ 4 of 6): TODO-{flux-heldout-verdict}. The clean-room auditor re-derived every completed gate with zero tier, metric or hash mismatches. One execution difference is recorded in the preregistration: the torch build on the GPU host changed since the original panel (diffusion library versions are pinned and identical), which can affect the strict pixel gates but not the route gates.

![Figure 4. A held-out prompt for the identity axis (squirrel → rabbit, seed 4242), written after the route was fixed. Injecting the route's state at dose 0, 0.5 and 1.0 moves the squirrel to a brown and then a grey rabbit matching the target; the wrong-axis donor changes color instead of species; ablating the route restores the squirrel. The full-dose energy-matched sham renders noise (see §9 on what that control does and does not rule out).](figures/fig4-heldout-prompt-identity.png)
...
- **Population statistics.** Four seeds have now been opened (two exploration, two held-out confirmation — §4); this is replication, not a distribution. Multi-seed necessity for the color contrast, in particular, is still single-seed (rescue is multi-seed). Prompt-held-out replication (new contrasts written after the route was fixed) has run for two of the six strict axes, both of which replicate at the route level (§4); the remaining four: TODO-{flux-heldout-groups-4-5-8}.
...
Known weaknesses of the current record, retained rather than hidden: strict dose thresholds were calibrated during exploration; the held-out-seed confirmation (§4) has since run with rules frozen in advance and its tier shifts are reported there, but the thresholds themselves still originate in exploration, and prompt-held-out contrasts are complete for two of six strict axes (TODO-{flux-heldout-groups-4-5-8}). The energy-matched sham, injected at full dose, destroys the image entirely (Figure 4; the original panel's shams do the same), so it rules out only that any perturbation of that size produces the target; the wrong-axis donor, which yields a coherent image that changes the wrong attribute, is the informative specificity control. The per-branch replay-exactness gate remains *metadata-based*: checkpoint tensors are not retained in the results tree, so replay declarations — while internally consistent and correctly consumed by the gate arithmetic — are attested by the same worker that produced the numbers; the clean-room auditor (§4) verifies everything derivable from retained artifacts but cannot re-execute a replay, and pixel recomputation proves recorded scalars are the correct function of recorded images, not that the images are genuine. Latent return-state metrics (the downstream-continuation gate and the return half of dose response), mediation-arm pixel distances, and the 0.25/0.75 dose points are likewise verifiable only as recorded scalars, their images not being retained. Edge mediation in the main panel is tested at one denoising step and one wrong-axis/sham realization per seed, not against full null distributions; the multi-step follow-up (§4) covers the strict axes at every step but leaves the route-certified axes single-step. Component byte hashes of the pinned revision are recorded for the language models but not yet for every FLUX component. And one scene run's submission record has an off-by-one in its branch count (17 declared per seed, 18 run — the extra being the route ablation).
...
## Appendix A.1: Where the evidence lives

| Experiment | Public artifacts in this repository |
|---|---|
| 20-axis certification panel, clean-room audit, promptless discovery | [`bfl/artifacts/twenty-axis-semantic-route-circuit/`](../../artifacts/twenty-axis-semantic-route-circuit/) (ledger, independent recheck, proof sheets, promptless control report) |
| Held-out-seed confirmation | [`data/heldout-seeds/`](data/heldout-seeds/) |
| Held-out-prompt replication | [`experiments/2026-09-22-prompt-heldout/`](experiments/2026-09-22-prompt-heldout/) (preregistration, results, ledger, independent recheck, montages); TODO-{flux-heldout-groups-4-5-8} |
| Multi-step mediation probe | [`data/mediation-steps/`](data/mediation-steps/) |
| Identity all-step vs single-step | [`papers/…/evidence/instrument-trial/bundle/case-1/`](../../../papers/pointwise-instruments-miss-distributed-circuits/evidence/instrument-trial/bundle/case-1/) |
| Scene certificate chain | [`bfl/artifacts/scene-circuit-certificate/`](../../artifacts/scene-circuit-certificate/) |
| Conditioner swap and conditioning-state repair | [`bfl/artifacts/cross-family-conditioner-repair/`](../../artifacts/cross-family-conditioner-repair/), [`bfl/artifacts/flux1-conditioner-causal-controls/`](../../artifacts/flux1-conditioner-causal-controls/) |
| Signal/response panel, linear and second-order controllers, fixed-dose baselines | [`data/response-panel/`](data/response-panel/) |
| Pythia-70M certificate | [`verifier/`](verifier/) (runs offline on CPU) |
| 12-axis panel and artifact-only recheck | internal report\* |
| Coordinate-law controls | internal report\* |
| Qwen2.5 / Qwen3 closure | internal report\* |
